...
<template>
  <section class="relative w-full h-[370px] overflow-hidden">
    <div v-for="(slide, index) in slides" :key="index" class="absolute inset-0 transition-opacity duration-1000" :class="{ 'opacity-100': currentSlide === index, 'opacity-0': currentSlide !== index }">
      ...
      <div class="absolute inset-0 bg-black/50 flex items-center justify-center text-white">
        <div class="text-center animate-slide-up">
          <h2 class="text-3xl md:text-4xl font-bold drop-shadow-lg">{{ slide.title }}</h2>
    ...
    <div class="absolute bottom-4 left-1/2 transform -translate-x-1/2 flex space-x-2">
      <button v-for="(slide, index) in slides" :key="index" @click="goToSlide(index)" class="w-3 h-3 rounded-full" :class="{ 'bg-amber-400': currentSlide === index, 'bg-white/50': currentSlide !== index }"></button>
    </div>
    <!-- Navigation Arrows -->
    <button @click="prevSlide" class="absolute cursor-pointer left-4 top-1/2 transform -translate-y-1/2 text-white text-3xl hover:text-amber-400">❮</button>
    <button @click="nextSlide" class="absolute cursor-pointer right-4 top-1/2 transform -translate-y-1/2 text-white text-3xl hover:text-amber-400">❯</button>
  </section>
</template>
