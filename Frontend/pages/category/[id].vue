...
const categoryName = categoryMap[categoryId] || "ไม่ทราบหมวดหมู่";

// กรองหนังสือตามหมวดหมู่
const filteredBooks = computed(() => {
  if (categoryId === "novel") {
    return books.filter((book) => book.category === "นิยาย");
  } else if (categoryId === "comic") {
    return books.filter((book) => book.category === "การ์ตูน");
  } else if (categoryId === "academic") {
    return books.filter((book) => book.category === "วิชาการ");
  } else if (categoryId === "others") {
    return books.filter(
      (book) =>
        !["นิยาย", "นิยายโรแมนติก", "การ์ตูน", "วิชาการ"].includes(
          book.category
        )
    );
  }
